...
(async (css, getURLSpecFromFile, loadText, versionGE, shouldSetIcon, isDef) => {
    ...
    window.addMenu = {
        ...
        sendAsyncMessage(key, data = {}, browser = gBrowser.selectedBrowser) {
        ...
        exec(path, arg, options = { blocking: false, startHidden: false }) {
        ...
        async moveToAppMenu(_e) {
            let ins = $('menuseparator#addMenu-app-insertpoint');
            if (ins) {
                ins.remove();
                ...
                await addMenu.rebuild();
            }
        },
        async rebuild(isAlert) {
            const aFile = this.FILE;
            ...
            function ps(item, arr) {
                Array.isArray(item) ? arr.push.apply(arr, item) :
                    arr.push(item);
            }
            ...
            try {
                ...
                Cu.evalInSandbox(includeSrc, sandbox);
            } catch (e) {
        ...
        newMenu(menuObj, opt = {}) {
            ...
            var isAppMenu = opt.insertPoint?.matches('toolbarseparator#addMenu-app-insertpoint'),
        ...
        newMenuitem(obj, opt = {}) {
            var menuitem,
                isAppMenu = opt.insertPoint?.matches('toolbarseparator#addMenu-app-insertpoint'),
                separatorType = isAppMenu ? "toolbarseparator" : "menuseparator",
        ...
        createMenuitem(itemArray, insertPoint) {
            ...
            for (let obj of itemArray) {
                ...
                if (sel && (menuitem = $(sel))) {
                    ...
                    if (!cls.contains('addMenu')) cls.add('addMenu');
        ...
        undoMods() {
            ...
            this.undoFunctions.length = 0;
        },
        removeMenuitem() {
            $$('.addMenuOriginal').forEach((e) => {
                let r = $(e.attr('original-id'));
                if (r) e.before(r);
                e.remove();
            });
        ...
        async setIcon(menu, obj) {
            if (menu.matches("[src],[icon]")) return;
        ...
        async _setExecIcon(menu, execPath) {
        ...
        async _setEngineIcon(menu, keyword) {
            try {
